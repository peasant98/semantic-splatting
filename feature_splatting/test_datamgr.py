...
@dataclass
class FeatureSplattingDataManagerConfig(DataManagerConfig):
...
class FeatureSplattingDataManager(DataManager, Generic[TDataset]):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # run the feature data manager setup
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        orig_class: Type[FeatureSplattingDataManager] = get_orig_class(self, default=None)  # type: ignore
        if type(self) is FeatureSplattingDataManager and orig_class is None:
            return default
        if orig_class is not None and get_origin(orig_class) is FeatureSplattingDataManager:
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            if get_origin(base) is FeatureSplattingDataManager:
    ...
    def next_eval(self, step: int) -> Tuple[Cameras, Dict]:
        camera, data = self.next_eval_image(step=step)
        camera_idx = camera.metadata['cam_idx']
        feature_dict = {}
